...
- For the sake of experimentation, let's assume the arc that forms the baseline of the logo  
  has a radius of 15 Blender units.
...
- Placement on the arc should happen before the shared X=20 degrees and Y=40 degrees rotations.
...
- Here is our sequence of steps for creating and shaping the logo:
  ...
  - Rotate the entire group of meshes positive 40 degrees around the Y axis, so the left side advances toward the camera while the right side recedes.
